...
chdir $RealBin;
my $path = "$RealBin/../bin/";
my $data = "$RealBin/data";
my $runner = SOBA::Run->new(path    => $path,
			    command => 'SOBAcl');
my $gff_file1 = "$data/dmel-all-r5.30_0001000.gff";

# The return value is the return value from the SOBAcl 0=success,
# 1=error thus: ok() to test failure and ok(!) to test success.
ok(! $runner->run(cl_args => '--help'), 'script complies');
like($runner->get_stdout, qr/Synopsis/, 'script prints usage statement');

#--------------------------------------------------------------------------------

# TEST ../bin/SOBAcl data/dmel-all-r5.30_0001000.gff >> sobacl_test.txt 2>> sobacl_test.error
my @cl_args = ("$data/dmel-all-r5.30_0001000.gff",
	      );

ok(! $runner->run(cl_args => \@cl_args), 'Basic run with defaults');
ok($runner->get_stdout =~ /\|match_part\s+\|445\s+\|/,
   'Results appear valid for basic run');
$runner->clean_up;

#--------------------------------------------------------------------------------

# TEST ../bin/SOBAcl --columns seqid  --rows type --data length --data_type mean  --layout table --format text data/dmel-all-r5.30_0001000.gff >> sobacl_test.txt 2>> sobacl_test.error
@cl_args = ('--columns seqid',
	    '--rows type',
	    '--data length',
	    '--data_type mean',
	    '--layout table',
...
ok(! $runner->run(cl_args => \@cl_args), 'Run SOBAcl with params (columns rows data data_type layout format');
ok($runner->get_stdout =~ /\|BAC_cloned_genomic_insert\s+\|166625\s+\|/,
   'Valid output');
$runner->clean_up;
